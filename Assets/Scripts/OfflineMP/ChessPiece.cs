...
public class ChessPiece : MonoBehaviour
{
    ...
    {
        if (!gameManager.GetComponent<GameManager>().IsGameOver() && gameManager.GetComponent<GameManager>().GetCurrentPlayer() == player)
        {
            DestroyMovePlates();
    ...
    public void InitiateMovePlates()
    {
        switch (this.name)
        {
            ...
            case "blackPawn":
                ...
                if (!moved && gameManager.GetComponent<GameManager>().GetPosition(xBoard, yBoard - 1) == null)
                {
    ...
    public void LineMovePlate(int xIncrement, int yIncrement)
    {
        ...
        if (gmScript.PositionOnBoard(x, y) && gmScript.GetPosition(x, y).GetComponent<ChessPiece>().player != player)
        {
    ...
    public void PawnMovePlate(int x, int y, bool canAttack)
    {
        ...
        if (gmScript.PositionOnBoard(x, y))
        {
            ...
            if (gmScript.PositionOnBoard(x + 1, y) && gmScript.GetPosition(x + 1, y) != null && 
                gmScript.GetPosition(x + 1, y).GetComponent<ChessPiece>().player != player && canAttack)
            {
    ...
    public void CastleMovePlate()
    {
        ...
        if (!moved && !inCheck)
        {
            if (gmScript.GetPosition(x + 1, y) == null && gmScript.GetPosition(x + 2, y) == null && gmScript.GetPosition(x + 3, y) != null)
            {
                ...
                {
                    if (rook.GetComponent<ChessPiece>().player == player && !rook.GetComponent<ChessPiece>().moved)
                    {
                        if (gmScript.IsLegalMove(this, x + 1, y, this, null, 0, 0) &&
                            gmScript.IsLegalMove(this, x + 2, y, this, null, 0, 0) &&
                            gmScript.IsLegalMove(this, x + 2, y, this, rook.GetComponent<ChessPiece>(), x + 1, y))
                        {
